...
import { formSchema } from "./FormEditBeer.form";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { FormEditBeerProps } from "./FormEditBeer.types";

export function FormEditBeer(props: FormEditBeerProps) {
  const { beerData, setOpenDialog } = props;
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      origin: beerData.origin,
      photo: beerData.photo,
      isPublish: beerData.isPublish ? beerData.isPublish : false,
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await axios.patch(`/api/beer/${beerData.id}/form`, values);
      toast({ title: "Beer edited ✌🏽" });
      router.refresh();
    } catch (error) {
      toast({
